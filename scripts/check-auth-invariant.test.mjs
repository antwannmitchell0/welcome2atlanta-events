...
test("the build side is deterministic without ignored .grok/app-env.json", () => {
  const missing = makeWorkspace();
  assert.equal(buildAuthEnabled(missing, {}), true);

  const shippedOff = makeWorkspace('{"VITE_AUTH_ENABLED":"false"}');
  assert.equal(buildAuthEnabled(shippedOff, {}), false);
  assert.equal(buildAuthEnabled(shippedOff, { VITE_AUTH_ENABLED: "true" }), true);

  const shippedOn = makeWorkspace('{"VITE_AUTH_ENABLED":"true"}');
  assert.equal(buildAuthEnabled(shippedOn, {}), true);
  assert.equal(buildAuthEnabled(shippedOn, { VITE_AUTH_ENABLED: "false" }), false);
});

test("this workspace does not require ignored .grok/app-env.json to resolve auth", () => {
  const resolved = buildAuthEnabled(projectRoot(), { VITE_AUTH_ENABLED: "true" });
