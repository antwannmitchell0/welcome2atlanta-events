...
test("the auth schema ships outside the globbed directory", () => {
  ...
  assert.ok(names.includes("0002_owner_portal.sql"));
  assert.ok(names.includes("0003_drop_storage_object.sql"));
  assert.ok(readdirSync(join(migrationsDir, "auth")).includes("0001_auth.sql"));
  const pending = pendingMigrations(names, []);
  assert.deepEqual(
    pending.map((item) => item.name),
    ["0001_auth.sql", "0002_owner_portal.sql", "0003_drop_storage_object.sql"],
  );
});
